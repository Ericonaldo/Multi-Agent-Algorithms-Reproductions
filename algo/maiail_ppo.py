...
class Discriminator(BaseModel):
    def __init__(self, sess, env, name=None, agent_id=None, entcoeff=0.001, lr=1e-2, units=128, e_w=1, a_w=1, grad_norm_clipping=0.5):
        ...
        self.expert_obs_phs = tf.placeholder(
            dtype=tf.float32, shape=(None,) + obs_space)
        self.expert_act_phs_n = [tf.placeholder(
            dtype=tf.float32, shape=(None,) + act_space[i]) for i in range(env.n)]
        expert_act_n = tf.concat(self.expert_act_phs_n, axis=1)
        self.expert_si_an = tf.concat(
            [self.expert_obs_phs, expert_act_n], axis=1)

        self.agent_obs_phs = tf.placeholder(
            dtype=tf.float32, shape=(None,) + obs_space)
        self.agent_act_phs_n = [tf.placeholder(
            dtype=tf.float32, shape=(None,) + act_space[i]) for i in range(env.n)]
        ...
        with tf.variable_scope('loss'):
            self.loss_expert = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
                logits=self.logit_1, labels=tf.ones_like(self.logit_1)))
            ...
            self.loss_agent = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
                logits=self.logit_2, labels=tf.zeros_like(self.logit_2)))
            ...
            gradients = optimizer.compute_gradients(
                self._loss, self.trainable_variables)
        ...
        self.expert_reward = tf.log(tf.nn.sigmoid(
            self.logit_1) + 1e-8) - tf.log(1 - tf.nn.sigmoid(self.logit_1) + 1e-8)
        # self.reward = tf.nn.sigmoid(self.logit_2)*2.0 - 1
        self.reward = tf.log(tf.nn.sigmoid(self.logit_2) + 1e-8) - \
            tf.log(1 - tf.nn.sigmoid(self.logit_2) + 1e-8)
        # self.reward = tf.log(tf.nn.sigmoid(self.logit_2)+1e-8)
    ...
    def _construct(self, input_ph, norm=False):
        l1 = tf.layers.dense(inputs=input_ph, units=self.units,
                             activation=tf.nn.leaky_relu, name='l1')
        if norm:
            l1 = tc.layers.layer_norm(l1)
        l2 = tf.layers.dense(inputs=l1, units=self.units,
                             activation=tf.nn.leaky_relu, name='l2')
        if norm:
            l2 = tc.layers.layer_norm(l2)
        l3 = tf.layers.dense(inputs=l2, units=self.units //
                             2, activation=tf.nn.leaky_relu, name='l3')
        out = tf.layers.dense(inputs=l3, units=1,
                              activation=tf.identity, name='prob')
    ...
    def train(self, expert_obs, expert_act_n, agent_obs, agent_act_n, alpha_i):
        ...
        loss, e_loss, a_loss, _ = self.sess.run(
            [self._loss, self.loss_expert, self.loss_agent, self._train_op], feed_dict=feed_dict)
...
class MAIAIL(BaseAgent):
        ...
        with tf.variable_scope(self.name):
            ...
            for i in range(self.n_agent):
                with tf.variable_scope("agent_{}".format(i)):
                    self.ppo.append(PPO(self.sess, env, name=name, agent_id=i, a_lr=lr,
                                        c_lr=lr, gamma=gamma, num_units=units, discrete=discrete))
            ...
            for i in range(self.n_agent):
                ...
                with tf.variable_scope("dicriminator_{}".format(i)):
                    self.disc.append(Discriminator(self.sess, env, name=name, agent_id=i,
                                                   entcoeff=entcoeff, lr=lr, units=units, grad_norm_clipping=grad_norm_clipping))
    ...
    def save(self, dir_path, iteration, max_to_keep):
        ...
        save_path = saver.save(self.sess, dir_name +
                               "/{}".format(self.name), global_step=iteration)
    ...
    def train(self, expert_dataset, learning_dataset, expert_pdf, agent_pdf, dm):
        ...
        for _ in range(self.d_step):  # train Discriminators d_step times
            ...
            for i in range(self.n_agent):
                ...
                # rho_1 = 1.0 * expert_pdf[i].prob(x)
                # rho_2 = 1.0 * self.disc[i].prob(x)
                ...
                alpha = self.lbd * np.clip(rho_1 / rho_2, 1e-1, 1. + 1e-1)
        ...
        for i in range(self.n_agent):
            ...
            print(
                "agent-reward-D: {}".format(self.disc[i].get_reward(obs_n[i], act_n)))
            obs_en, act_en = expert_dataset.sample(5)
            # print("expert-(s,a): ({},{})".format(obs_en, act_en))
            print(
                "expert-reward-D: {}".format(self.disc[i].get_expert_reward(obs_en[i], act_en)))

        # compute rewards and gaes
        learning_dataset.compute(
            self.gamma, [self.disc[i].get_reward for i in range(self.n_agent)], True)
        ...
        for i in range(self.n_agent):
            ...
            for _ in range(self.p_step):  # train policy 6 times
                obs, actions, rewards, v_preds, v_preds_next, gaes = learning_dataset.sample(
                    self.batch_size)
                t_info = self.ppo[i].train(
                    obs[i], actions[i], rewards[i], v_preds_next[i], gaes[i])
