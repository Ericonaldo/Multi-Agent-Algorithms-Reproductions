...
class Discriminator(BaseModel):
    def __init__(self, sess, expert_si_ai, agent_si_ai, entcoeff=0.001, lr=1e-2, name=None, agent_id=None, units=128, e_w=1, a_w=1):
        super().__init__(name)
        ...
        with tf.variable_scope('loss'):
            self.loss_expert = e_w * tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.logit_1, labels=tf.ones_like(self.logit_1)))
            # loss_expert = tf.reduce_mean(tf.log(tf.nn.sigmoid(self.prob_1)+1e-8))
            self.loss_agent = a_w * tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.logit_2, labels=tf.zeros_like(self.logit_2)))
...
class MADiscriminator(object):
    def __init__(self, sess, env, scenario, name, n_agent, expert_dataset, batch_size=512, entcoeff=0.001, lr=1e-2, memory_size = 10**4, units=128, lbd=1, e_w=1, a_w=1, lower_dimension=None):
        ...
        self.expert_act_n = self.expert_act_phs_n
        self.expert_si_ai = [tf.concat([self.expert_obs_n[i], self.expert_act_n[i]], axis=1) for i in range(n_agent)]
        ...
        self.agent_act_n = self.agent_act_phs_n
        ...
        with tf.variable_scope(self.name):
            for i in range(self.env.n):
                ...
                with tf.variable_scope("dicriminator_{}".format(i)):
                    self.discriminators.append(Discriminator(self.sess, self.expert_si_an[i], self.agent_si_an[i], lr=lr, name=name, agent_id=i, units=units, e_w=e_w, a_w=a_w))
