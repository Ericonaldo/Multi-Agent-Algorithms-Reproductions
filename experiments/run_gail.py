...
if __name__ == '__main__':
    parser = argparse.ArgumentParser("GAIL experiments")
    # Environment
    ...
    parser.add_argument("--bc_init", action="store_true", default=False, help="need to be initialized with bc")
    parser.add_argument("--p_step", type=int, default=4, help="training times of policy network")
    parser.add_argument("--d_step", type=int, default=1, help="training times of discriminator")
    ...
    parser.add_argument("--exp_name", type=str, default="gail", help="name of the experiment")
    parser.add_argument("--save_interval", type=int, default=1, help='Interval episode for saving model(default=400)')
    ...
    parser.add_argument("--max_to_keep", type=int, default=0, help="number of models to save")
    # Evaluation
    ...
    gail = GAIL(sess, env, args.scenario, exp_name, num_agents, args.batch_size, args.entcoeff, args.lr, args.gamma, args.p_step, args.d_step, args.units, args.discrete)
    ...
    if not is_evaluate:
        ...
        gail.init()  # run self.sess.run(tf.global_variables_initializer())

    if args.restore or is_evaluate:
        load_dir = os.path.join(args.load_dir, args.scenario)
        gail.load(load_dir, epoch=args.load_epoch)
    ...
    if not is_evaluate:
        ...
        if args.bc_init:
            gail.bc_init(100, expert_dataset)
    else:
    ...
    for iteration in range(iterations):
        ...
        if not is_evaluate:
            ...
            t_info = gail.train(expert_dataset, learning_dataset) 
            ...
            if (iteration+1) % args.save_interval == 0:
                gail.save(args.save_dir+args.scenario, iteration, args.max_to_keep)
